#!/bin/bash

################################################################################
# hvault_simple.sh
# 
# SIMPLE VERSION - No environment in file needed!
# 
# File format (SIMPLE):
# oracle_name=PATH/PLACEHOLDER
# instance_name:INSTANCENAME
#
# Script builds Vault path automatically:
# {VAULT_BASE_PATH}/{INSTANCENAME}
#
# Example usage:
# ./hvault_simple.sh -i file.conf -b "oracle/qua/QA"
# ./hvault_simple.sh -i file.conf -b "oracle/qua/PROD"
################################################################################

set -euo pipefail

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

# Counters
SUCCESS_COUNT=0
FAIL_COUNT=0

# Configuration
VAULT_ADDR="${VAULT_ADDR:-}"
VAULT_TOKEN="${VAULT_TOKEN:-}"
VAULT_NAMESPACE="${VAULT_NAMESPACE:-AP11436}"
VAULT_SKIP_VERIFY="${VAULT_SKIP_VERIFY:-false}"
MOUNT="cloud"
INPUT_FILE=""
VAULT_BASE_PATH="oracle/qua/QA"  # DEFAULT - can be overridden with -b flag

# Debug mode
DEBUG="${DEBUG:-false}"

################################################################################
# LOGGING
################################################################################

log_info() {
    echo -e "${BLUE}[INFO]${NC} $1" >&2
}

log_success() {
    echo -e "${GREEN}[✓]${NC} $1" >&2
}

log_warn() {
    echo -e "${YELLOW}[⚠]${NC} $1" >&2
}

log_error() {
    echo -e "${RED}[✗]${NC} $1" >&2
}

log_debug() {
    if [[ "$DEBUG" == "true" ]]; then
        echo -e "${BLUE}[DEBUG]${NC} $1" >&2
    fi
}

usage() {
    cat <<EOF
Usage: $0 -i INPUT_FILE [-b VAULT_BASE_PATH] [OPTIONS]

Required Arguments:
  -i INPUT_FILE             Static connections file

Optional Arguments:
  -b VAULT_BASE_PATH        Vault base path (default: oracle/qua/QA)
                            Examples:
                              oracle/qua/QA
                              oracle/qua/PROD
                              oracle/qua/PREPRODUCTION
  -m MOUNT_POINT            Vault mount point (default: cloud)
  -h, --help                Show this help message

Environment Variables:
  VAULT_ADDR                Vault server address (required)
  VAULT_TOKEN               Vault authentication token (required)
  VAULT_NAMESPACE           Vault namespace (default: AP11436)
  VAULT_SKIP_VERIFY         Skip SSL verification (default: false)
  DEBUG                     Enable debug mode (default: false)

Input File Format (SIMPLE - no environment needed!):
  oracle_name=PATH/PLACEHOLDER
  instance_name:INSTANCENAME

Examples:
  oracle=OWN_11436_WAREHOUSE/PLACEHOLDER
  instance:Q143652DP10
  
  oracle_dnk=WAREHOUSE/PLACEHOLDER
  instance_dnk:Q143652EP10

Usage Examples:
  # For QA environment
  ./hvault_simple.sh -i static_connections.conf -b "oracle/qua/QA"
  
  # For PROD environment
  ./hvault_simple.sh -i static_connections.conf -b "oracle/qua/PROD"
  
  # For PREPRODUCTION
  ./hvault_simple.sh -i static_connections.conf -b "oracle/qua/PREPRODUCTION"

Output Format (same as input, with passwords):
  oracle_name=PATH/""PASSWORD""
  instance_name:INSTANCENAME

EOF
    exit 1
}

################################################################################
# VALIDATION
################################################################################

validate_environment() {
    if [[ -z "$VAULT_ADDR" ]]; then
        log_error "VAULT_ADDR environment variable not set"
        exit 1
    fi
    
    if [[ -z "$VAULT_TOKEN" ]]; then
        log_error "VAULT_TOKEN environment variable not set"
        exit 1
    fi
}

validate_files() {
    if [[ -z "$INPUT_FILE" ]]; then
        log_error "Input file (-i) is required"
        usage
    fi
    
    if [[ ! -f "$INPUT_FILE" ]]; then
        log_error "File not found: $INPUT_FILE"
        exit 1
    fi
}

################################################################################
# VAULT OPERATIONS
################################################################################

fetch_password_from_vault() {
    local instance="$1"
    
    # Construct full Vault path
    local vault_path="${VAULT_BASE_PATH}/${instance}"
    
    log_debug "Fetching password for instance: $instance"
    log_debug "Vault path: $vault_path"
    
    local curl_opts=(-s -S)
    [[ "$VAULT_SKIP_VERIFY" == "true" ]] && curl_opts+=(-k)
    
    local response
    response=$(curl "${curl_opts[@]}" \
        -H "X-Vault-Token: ${VAULT_TOKEN}" \
        -H "X-Vault-Namespace: ${VAULT_NAMESPACE}" \
        "${VAULT_ADDR}/v1/${VAULT_NAMESPACE}/${MOUNT}/data/${vault_path}" 2>&1) || {
        log_error "Failed to reach Vault for instance $instance (path: $vault_path)"
        return 1
    }
    
    local password
    password=$(echo "$response" | jq -r '.data.data.password // empty' 2>/dev/null) || {
        log_error "Failed to parse Vault response for instance $instance"
        log_debug "Response: $response"
        return 1
    }
    
    [[ -z "$password" ]] && {
        log_error "No password found in Vault at path: $vault_path"
        log_debug "Response: $response"
        return 1
    }
    
    echo "$password"
    return 0
}

################################################################################
# EXTRACTION FUNCTIONS
################################################################################

# Extract instance name from "instance_name:INSTANCENAME"
extract_instance() {
    local line="$1"
    echo "$line" | sed 's/.*://'
}

# Check if line is a connection line
is_connection_line() {
    local line="$1"
    [[ "$line" =~ ^oracle_ ]] && [[ "$line" =~ /PLACEHOLDER$ ]]
}

# Check if line is an instance line
is_instance_line() {
    local line="$1"
    # Format: instance_XXX:INSTANCENAME (no environment!)
    [[ "$line" =~ ^instance_[^:]+:[A-Z0-9]+$ ]]
}

# Replace /PLACEHOLDER with /""PASSWORD""
inject_password() {
    local connection_line="$1"
    local password="$2"
    
    echo "$connection_line" | sed "s|/PLACEHOLDER|/\"\"${password}\"\"|"
}

################################################################################
# MAIN PROCESSING
################################################################################

process_file() {
    local file="$1"
    local line_num=0
    local pending_connection=""
    local in_comment_section=false
    
    log_info "Processing file: $file"
    log_info "Using Vault base path: $VAULT_BASE_PATH"
    
    while IFS= read -r line || [[ -n "$line" ]]; do
        ((line_num++))
        
        log_debug "Line $line_num: $line"
        
        # Handle comment/header lines
        if [[ "$line" =~ ^# ]] || [[ "$line" =~ ^= ]]; then
            log_debug "Line $line_num: Comment/header"
            echo "$line"
            in_comment_section=true
            continue
        fi
        
        # Empty lines
        if [[ -z "$line" ]]; then
            log_debug "Line $line_num: Empty line"
            if [[ "$in_comment_section" == "true" ]]; then
                echo "$line"
            fi
            continue
        fi
        
        in_comment_section=false
        
        # Connection line
        if is_connection_line "$line"; then
            log_debug "Line $line_num: Connection line detected"
            pending_connection="$line"
            continue
        fi
        
        # Instance line
        if is_instance_line "$line"; then
            log_debug "Line $line_num: Instance line detected"
            
            if [[ -z "$pending_connection" ]]; then
                log_warn "Line $line_num: Instance line without connection line"
                echo "$line"
                continue
            fi
            
            # Extract instance
            local instance
            instance=$(extract_instance "$line")
            log_debug "Extracted instance: $instance"
            
            # Fetch password
            local password
            if ! password=$(fetch_password_from_vault "$instance"); then
                log_error "Line $line_num: Failed to fetch password for $instance"
                echo "$pending_connection"
                echo "$line"
                ((FAIL_COUNT++))
                pending_connection=""
                continue
            fi
            
            # Inject password
            local updated_connection
            updated_connection=$(inject_password "$pending_connection" "$password")
            
            log_success "Line $line_num: Password injected for $instance"
            
            # Output both lines
            echo "$updated_connection"
            echo "$line"
            
            ((SUCCESS_COUNT++))
            pending_connection=""
            continue
        fi
        
        # Other lines (preserve as-is)
        log_debug "Line $line_num: Other line"
        echo "$line"
        
    done < "$file"
    
    # Summary
    echo "" >&2
    log_info "Processing Complete:"
    log_success "  Successfully processed: $SUCCESS_COUNT connections"
    [[ $FAIL_COUNT -gt 0 ]] && log_error "  Failed: $FAIL_COUNT connections"
}

################################################################################
# MAIN
################################################################################

main() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            -i)
                INPUT_FILE="$2"
                shift 2
                ;;
            -b)
                VAULT_BASE_PATH="$2"
                shift 2
                ;;
            -m)
                MOUNT="$2"
                shift 2
                ;;
            -h|--help)
                usage
                ;;
            *)
                log_error "Unknown option: $1"
                usage
                ;;
        esac
    done
    
    validate_environment
    validate_files
    
    log_info "╔════════════════════════════════════════════════════╗"
    log_info "║  hvault_simple.sh - Simple Password Injector      ║"
    log_info "╚════════════════════════════════════════════════════╝"
    
    process_file "$INPUT_FILE"
    
    if [[ $FAIL_COUNT -gt 0 ]]; then
        exit 1
    fi
    
    exit 0
}

main "$@"
